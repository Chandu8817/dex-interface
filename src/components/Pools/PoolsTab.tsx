...
export function PoolsTab({ signer,handleChangeSetActiveTab }: PoolsTabProps ) {
  ...
  const { getPoolAddress, createPool } = useFactory(
    signer as ethers.JsonRpcSigner
  );
  const {getUserPositions} = usePositionManager(signer as ethers.JsonRpcSigner);
  const { initializePool, isPoolInitialized } = usePool(
    signer as ethers.JsonRpcSigner
  );
    ...
    const fetchUserPools = async () => {
      if (signer) {
        try {
          ...
          const detailedPositions = await Promise.all(positions.map(async (pos) => {
...
debugger
      const { txHash, newPoolAddress } = await createPool(
        sortedToken0,
        sortedToken1,
        feeTier
      );
  ...
  const handleAddLiquidity = (pool: Pool) => {
    try {
     ...
     handleChangeSetActiveTab("liquidity");
    } catch (error) {
      ...
      <div className="flex flex-col space-y-4">
        ...
        <div className="space-y-4">
          ...
          <div className="w-full md:w-1/2">
            ...
            >
              <option value={100}>
                0.01% - Best for stable pairs (e.g., USDC/USDT)
              </option>
              <option value={500}>0.05% - Best for stable pairs</option>
              <option value={3000}>
                0.3% - Best for most pairs (e.g., ETH/USDC)
              </option>
      ...
      <div className="mt-8">
        ...
        {isLoading ? (
          ...
          <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 rounded-lg">
           {activeTab==="all"? <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <div className="flex-shrink-0 h-10 w-10 flex -space-x-2">
                          <img
                            ...
                            onError={(e) => {
                              ...
                              target.src = "https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/info/logo.png";
                            }}
                          />
                          <img
                            className="h-8 w-8 rounded-full border-2 border-white dark:border-gray-800"
        ...
        <td className="px-6 py-4 whitespace-nowrap">
          <div className="flex items-center">
            <div className="flex-shrink-0 h-10 w-10 flex -space-x-2">
              <img
                className="h-8 w-8 rounded-full border-2 border-white dark:border-gray-800"
                src={pos.token0Logo || "https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/info/logo.png"}
              ...
              <img
                className="h-8 w-8 rounded-full border-2 border-white dark:border-gray-800"
                src={pos.token1Logo || "https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/info/logo.png"}
